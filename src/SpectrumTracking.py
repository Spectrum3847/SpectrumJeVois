...
##################################################################################################
        ...
        if (contourNum >= numberOfTargets)
            ...
            for i in range (numberOfTargets):
                ...
                tx = (tx + round((getXcoord(cnt)*1000/320)-500, 2)) / (i+1) # x-coordinate of contour, -500 to 500 rounded to 2 decimal
                ty = (ty + round(375-getYcoord(cnt)*750/240, 2)) / (i+1) # y-coordinate of contour, -375 to 375 rounded to 2 decimal
        ...
        # Write a title:
        cv2.putText(outimg, "SpectrumCV", (width/2, 20), cv2.FONT_HERSHEY_SIMPLEX, 0.3, (255,255,255), 1.5, cv2.LINE_AA)
