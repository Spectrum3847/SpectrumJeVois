...
#
# This module is a basic FRC vision process for Spectrum FRC#3847
# This has been updated to allow getting the combined center of 1 or more targets.
#
#
# This package is based heavily on the work of Anand Rajamani his comments are below. Please note this code was modified
...
#
# @videomapping MJPG 320 240 30 YUYV 320 240 60 JeVois SpectrumTracking
...
##################################################################################################
        ...
        if contourNum >= numberOfTargets:
            ...
            for i in range (numberOfTargets):
                ...
                ta += getArea(cnt); #Currently just returns the combined area of the targest
                #calculate the center of the targets
                tx = round((tx + (getXcoord(cnt)*1000/320)-500) / (i+1),2) # x-coordinate of contour, -500 to 500 rounded to 2 decimal
                ty = round((ty + 375-getYcoord(cnt)*750/240) / (i+1), 2) # y-coordinate of contour, -375 to 375 rounded to 2 decimal
        ...
        pixels = {"tx" : tx, "ty" : ty, "ta" : ta, "tv" : tv}
        json_pixels = json.dumps(pixels)
        jevois.sendSerial(json_pixels)
        #height, width, channels = outimg.shape # if outimg is grayscale, change to: height, width = outimg.shape
        height, width, channels = outimg.shape
        # Write a title:
        cv2.putText(outimg, "SpectrumCV", (int(width - 80), 20), cv2.FONT_HERSHEY_SIMPLEX, 0.3, (255,255,255), 1, cv2.LINE_AA)
